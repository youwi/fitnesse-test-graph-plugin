...
public class FileResponder implements SecureResponder {
    ...
    private Response makeClasspathResponse(FitNesseContext context, Request request) throws Exception {
        ...
        while (!reader.isEof()) {
            byte[] temp = reader.readBytes(RESOURCE_SIZE_LIMIT);
            byte[] both = Arrays.copyOf(content, content.length + temp.length);
            System.arraycopy(temp, 0, both, content.length, temp.length);
            content = both;
        }
