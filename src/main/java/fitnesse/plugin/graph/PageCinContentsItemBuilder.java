...
public class PageCinContentsItemBuilder  extends ContentsItemBuilder {
    private Symbol contents;
    private int level;
    private SourcePage page;

    public PageCinContentsItemBuilder(Symbol contents, int level) {
        this(contents, level, null);
    }

    public PageCinContentsItemBuilder(Symbol contents, int level, SourcePage page) {
        super(contents,level,page);
    ...
    private HtmlTag buildListItem(SourcePage child) {
        ...
        if (!child.getChildren().isEmpty()) {
            if (level < getRecursionLimit()) {
                listItem.add(new PageCinContentsItemBuilder(contents, level + 1, child).buildLevel(child));
            }
    ...
    public HtmlTag buildItem(SourcePage page) {
        ...
        listItem.add(link);
        if (!help.isEmpty()) {
            listItem.add(HtmlUtil.makeSpanTag("pageHelp", ": " + page.getName()));
            link.use(help);
